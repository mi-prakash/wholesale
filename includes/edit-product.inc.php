...
if (isset($_POST)) {
    if (isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
        ...
        $product = new ProductContr($name, $retail_price, $wholesale_price, $publish_by, $status, $updated_at);

        // Running error handlers and user product
        $update_product = $product->updateProduct($id);

        if (isset($update_product["status"]) && $update_product["status"] == "error") {
            $response = json_encode($update_product);
            echo $response;
            exit;
        }
        ...
        if($img1_failed == true || $img2_failed == true) {
            if($img1_failed == true && $img2_failed == false) {
                $response = array(
                    "status" => "success",
                    "message" => "Product successfully updated. Failed to update Image",
                    "data" => $product_detail
                );
            } elseif ($img1_failed == false && $img2_failed == true) {
                $response = array(
                    "status" => "success",
                    "message" => "Product successfully updated. Failed to update Image (Optional)",
                    "data" => $product_detail
                );
            } else {
                $response = array(
                    "status" => "success",
                    "message" => "Product successfully updated. Failed to update Image & Image (Optional)",
                    "data" => $product_detail
                );
            }
        } else {
